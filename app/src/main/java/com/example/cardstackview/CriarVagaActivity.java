...
public class CriarVagaActivity extends AppCompatActivity {
    ...
    private void enviarParaPreVisualizacao() {
        ...
        // Criar objeto Vaga usando o construtor vazio e setters
        ...
        vaga.setNivel_experiencia(nivel);
        vaga.setTipo_contrato(contrato);
        vaga.setArea_atuacao(area);
        vaga.setBeneficios(beneficios);
        vaga.setVinculo(contrato); // Usando o mesmo valor do tipo de contrato
        vaga.setRamo("Tecnologia"); // Definir ramo ou adicionar campo no formulário
        vaga.setId_usuario(userId);
        vaga.setHabilidadesDesejaveisStr(String.join(", ", habilidadesDesejaveis));

        // Definir valores padrão para campos não preenchidos no formulário
        vaga.setVaga_id(0); // Será definido pelo banco de dados
        vaga.setEmpresa_id(0); // Pode ser ajustado conforme necessário
        vaga.setNome_empresa(""); // Pode ser preenchido posteriormente
